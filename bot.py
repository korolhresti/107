...
from croniter import croniter # Для планування завдань

# Імпорт заглушок парсерів
import web_parser
...
@dp.message(AddSourceStates.waiting_for_url)
...
@dp.callback_query(AddSourceStates.waiting_for_type, F.data.startswith("source_type_"))
...
async def handle_my_news_command(callback: CallbackQuery):
    ...
    await dp.fsm.get_context(
        bot=bot,
        key=types.Chat(id=callback.message.chat.id, type=callback.message.chat.type).as_json(), # <--- ВИПРАВЛЕНО: Додано type
        user=callback.from_user.id
    ).update_data(current_news_index=0, news_ids=all_news_ids)

    current_news_id = news_items[0].id
    await send_news_to_user(callback.message.chat.id, current_news_id, 0, len(all_news_ids))
    await callback.answer()

@dp.callback_query(F.data == "next_news")
async def handle_next_news_command(callback: CallbackQuery):
    user = await get_user_by_telegram_id(callback.from_user.id)
    if not user:
    ...
    state_context = dp.fsm.get_context(
        bot=bot,
        key=types.Chat(id=callback.message.chat.id, type=callback.message.chat.type).as_json(), # <--- ВИПРАВЛЕНО: Додано type
        user=callback.from_user.id
    )
    user_data = await state_context.get_data()
    ...
    await state_context.update_data(current_news_index=next_index)

    await send_news_to_user(callback.message.chat.id, next_news_id, next_index, len(news_ids))
    await callback.answer()

async def send_news_to_user(chat_id: int, news_id: int, current_index: int, total_news: int):
    ...
    nav_buttons = []
    if current_index > 0:
        nav_buttons.append(InlineKeyboardButton(text="⬅️ Попередня", callback_data="prev_news")) # Можна додати prev_news
...
@dp.message("waiting_for_ai_question")
async def process_ai_question(message: Message, state: FSMContext):
    user_question = message.text
    user_data = await state.get_data()
    news_id = user_data.get("current_news_id_for_ai")
...
@dp.callback_query(F.data.startswith("extract_entities_"))
...
@dp.callback_query(F.data.startswith("explain_term_"))
async def handle_explain_term(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state("waiting_for_term_explanation")
    await callback.answer()

@dp.message("waiting_for_term_explanation")
...
@dp.callback_query(F.data.startswith("classify_topics_"))
...
@dp.callback_query(F.data.startswith("fact_check_news_"))
...
@dp.callback_query(F.data.startswith("sentiment_trend_analysis_"))
...
@dp.callback_query(F.data.startswith("bias_detection_"))
...
@dp.callback_query(F.data.startswith("audience_summary_"))
...
@dp.callback_query(F.data.startswith("historical_analogues_"))
...
@dp.callback_query(F.data.startswith("impact_analysis_"))
...
@dp.callback_query(F.data.startswith("what_if_scenario_"))
...
@dp.callback_query(F.data.startswith("report_fake_news_"))
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        # aiogram 3.x очікує повний об'єкт Update, тому model_validate
